...
// ── Config ────────────────────────────────────────────────────────────────────
const FORM_DURATION = 300 // 5 minutes in seconds
...
function fingerprint() {
  try {
    return btoa([navigator.userAgent, navigator.language, screen.width, screen.height].join('|')).slice(0, 64)
...
export default function QRScanPage() {
  ...
  // ── Step 1: validate QR token on mount ──────────────────────────────────────
  useEffect(() => {
    async function validate() {
      ...
      if (!qr || !qr.is_active || new Date(qr.expires_at) < new Date()) {
        setPageState('qr_expired'); return
      }
  ...
  // ── Step 2: 5-minute form countdown ─────────────────────────────────────────
  useEffect(() => {
    if (pageState !== 'valid') {
  ...
  if (pageState === 'form_expired') return <StatusScreen icon="⏳" title="Form timed out" message="The 5-minute window has passed. Please scan the QR code again to get a fresh form." accent="amber" />
  ...
  if (pageState === 'duplicate') return <StatusScreen icon="✅" title="Already recorded" message="Your attendance for this session is already saved. No further action needed." accent="green" />
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="lg:w-[380px] lg:min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-indigo-950
        flex flex-col px-8 py-10 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-3 mb-10">
          <div className="w-10 h-10 bg-indigo-500/20 border border-indigo-400/30 rounded-xl flex items-center justify-center text-xl">
            📋
          </div>
          <div>
        ...
        {/* 5-minute countdown */}
        <div className={`relative z-10 mt-8 rounded-2xl p-5 border transition-colors ${
          timerUrgent ? 'bg-red-900/30 border-red-500/30' : 'bg-white/5 border-white/10'
        }`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-3xl font-bold tabular-nums ${timerUrgent ? 'text-red-400' : 'text-white'}`}>
              {fmt(timeLeft)}
            </span>
          </div>
          ...
          <p className={`text-xs mt-2.5 ${timerUrgent ? 'text-red-400' : 'text-slate-500'}`}>
            Submit before this timer runs out. If it expires, rescan the QR code.
          </p>
        </div>

        {/* Security note */}
        <p className="relative z-10 mt-5 text-xs text-slate-600 flex items-start gap-2">
          <svg className="w-4 h-4 shrink-0 mt-0.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
          </svg>
          QR codes refresh every few seconds. Screenshot reuse is automatically blocked.
        </p>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          {cached?.full_name && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-4 py-3 mb-6
              ...
              <button type="button"
                onClick={() => { setCached(null); setForm({ full_name: '', phone: '', email: '', institution: '', designation: '' }) }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <Field label="Phone number" required error={errors.phone}>
              <input type="tel" value={form.phone}
                ...
                placeholder="+232 76 000 000" autoComplete="tel"
            ...
            <button type="submit" disabled={submitting}
              className="w-full bg-indigo-600 text-white py-3.5 rounded-2xl text-sm font-semibold
                hover:bg-indigo-700 active:scale-[0.99] transition-all
                disabled:opacity-50 disabled:cursor-not-allowed
                shadow-lg shadow-indigo-200/60 mt-2">
              {submitting
          ...
          <p className="text-xs text-center text-gray-400 mt-5">
            Your details are stored securely and used only for this event&apos;s attendance records.
          </p>
        </div>
      </div>
    </div>
