...
function AttendForm() {
  ...
  useEffect(() => {
    async function validate() {
      ...
      // Token must exist AND be active (not rotated out)
      if (!qr || !qr.is_active) { setPageState('qr_expired'); return }

      // Token expiry as secondary check (rotation sets short expiry)
  ...
  if (pageState === 'qr_expired') return (
    <StatusScreen
      icon="⏱️"
      title="QR code expired"
      message="This QR code has expired or been replaced. Please scan the latest code displayed at the venue."
      accent="amber"
    />
  )
  if (pageState === 'form_expired') return (
    <StatusScreen
      icon="⏳"
      title="Form timed out"
      message="The 5-minute window has passed. Please scan the QR code again to get a fresh form."
      accent="amber"
    />
  )
  if (pageState === 'closed') return (
    <StatusScreen
      ...
      message="This session has ended. The organiser is no longer accepting check-ins."
      accent="slate"
    />
  )
  if (pageState === 'duplicate') return (
    <StatusScreen
      ...
      message="Your attendance for this session is already saved. No further action needed."
      accent="green"
    />
  )
  if (pageState === 'success') return (
    <StatusScreen
      ...
      message="Your attendance has been recorded successfully. You can close this page."
      accent="green"
    />
  )
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      {/* LEFT — dark info panel (hidden on mobile, shows as top bar) */}
      <div className="lg:w-[360px] lg:min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-indigo-950 flex flex-col px-6 py-8 lg:px-8 lg:py-10 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-3 mb-8">
          <div className="w-9 h-9 bg-indigo-500/20 border border-indigo-400/30 rounded-xl flex items-center justify-center text-lg shrink-0">📋</div>
          <div>
        ...
        <div className="relative z-10 flex-1">
          <p className="text-xs font-semibold text-indigo-400 uppercase tracking-widest mb-2">Checking in for</p>
          <h1 className="text-xl lg:text-2xl font-bold leading-snug mb-1">{event?.name ?? '—'}</h1>
          <p className="text-slate-400 text-sm">{session?.name}</p>
          {event?.location && (
        ...
        {/* Timer */}
        <div className={`relative z-10 mt-6 rounded-2xl p-4 border transition-colors ${timerUrgent ? 'bg-red-900/30 border-red-500/30' : 'bg-white/5 border-white/10'}`}>
          ...
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-1000 ${timerUrgent ? 'bg-red-500' : 'bg-indigo-400'}`}
              style={{ width: `${timerPct}%` }}
            />
          </div>
          <p className={`text-xs mt-2 ${timerUrgent ? 'text-red-400' : 'text-slate-500'}`}>
            Submit before this timer runs out.
          </p>
        </div>

        {/* Security note */}
        <p className="relative z-10 mt-4 text-xs text-slate-600 flex items-start gap-2">
          <svg className="w-3.5 h-3.5 shrink-0 mt-0.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
          </svg>
          QR codes refresh periodically. Screenshot reuse is automatically blocked.
        </p>
      </div>

      {/* RIGHT — form panel */}
      <div className="flex-1 bg-gray-50 flex items-start lg:items-center justify-center px-4 sm:px-6 py-8 lg:py-16 min-h-0">
        <div className="w-full max-w-md">
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-2xl px-4 py-3 mb-5 flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-indigo-800">
                  Welcome back, {cached.full_name.split(' ')[0]} 👋
                </p>
                <p className="text-xs text-indigo-500 mt-0.5">
                  Your details are pre-filled from your last visit — review and submit.
                </p>
              </div>
              <button
                type="button"
                ...
                className="text-xs text-indigo-500 hover:text-indigo-700 underline shrink-0 mt-0.5 whitespace-nowrap">
                Not you?
              </button>
            </div>
          )}
          ...
          <p className="text-xs text-center text-gray-400 mt-5">
            Your details are saved on this device for future visits. Tap &quot;Not you?&quot; to clear them.
          </p>
        </div>
      </div>
    </div>
...
function StatusScreen({ icon, title, message, accent }: {
  icon: string; title: string; message: string; accent: 'amber' | 'green' | 'slate'
}) {
  const bg = {
    amber: 'from-amber-900/40 to-slate-900',
    green: 'from-emerald-900/40 to-slate-900',
...
function inputCls(hasError: boolean) {
  return [
    'w-full border rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400',
    'focus:ring-2 focus:border-transparent outline-none transition-shadow',
    hasError
      ? 'border-red-300 focus:ring-red-400 bg-red-50/50'
      : 'border-gray-200 focus:ring-indigo-500 bg-white hover:shadow-sm',
  ].join(' ')
}